...
![Figure: Time trimming example](time-trimming-example.png)
...
![Figure: Merge button](merge-button.png)

![Figure: Merge Datasets dialog](merge-datasets-dialog.png)
...
- **Apply resampling** allows manually changing (increase) the duration of sampling intervals to match sampling intervals of multiple datasets for merging.

>**Note**: It is not possible to decrease the original sampling interval. Therefore, a sampling interval needs to be adjusted to match with the dataset with the longest sampling interval.
{style='note'}
...
-
- **Trim time** allows shortening a dataset by adjusting the start and end time, and thereby matching the time frames and duration of individual experiments for merging.
...
> **Note**: Datasets can only be merged if the lists of variables are identical.
